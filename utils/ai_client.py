...
### 写作格式
- 当你新写小说正文时（包括续写），请用 **【小说内容】** 和 **【/小说内容】** 标签包围正文，这段内容会追加到已有内容之后
- 当你修改/替换小说内容时（用户要求修改某处、删除某段、重写某章等），请用 **【替换内容】** 和 **【/替换内容】** 标签，标签内必须包含修改后的完整正文（不是只给修改的部分），这段内容会完全替换已有内容
- 正文之外的讨论、建议、询问等不要用此标签
- 每次写作保持在适当的长度（800-2000字为宜），除非用户要求更长或更短
